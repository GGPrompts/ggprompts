...
import { Bot, ArrowLeft, Filter } from 'lucide-react'
...
export default async function AgentsPage({ searchParams }: AgentsPageProps) {
  ...
  const { data: categories } = await supabase
    ...
    .select('*')
    .or('component_type.eq.agent,component_type.is.null')
    .order('sort_order')
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <Badge variant="secondary">{agents?.length || 0}</Badge>
        </div>

        <p className="text-muted-foreground">
          Specialized Claude personas with custom system prompts and tool restrictions
        </p>
      </div>
      ...
      <div className="glass border-border/50 rounded-xl p-4 mb-8">
        <form className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {categories?.map((cat) => (
              <Link key={cat.slug} href={`/claude-code/agents?category=${cat.slug}`}>
                <Badge
      ...
      {agents && agents.length > 0 ? (
        <ComponentGrid
          components={agents as Component[]}
        ...
        <div className="text-center py-20">
          ...
          <p className="text-muted-foreground">
            {params.q || params.category
              ...
              : 'Check back soon for new agents!'}
          </p>
        </div>
      )}
    </div>
