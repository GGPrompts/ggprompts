...
export default async function AgentDetailPage({ params }: AgentDetailPageProps) {
  ...
  if (user) {
    const { data: toolkit } = await supabase
      ...
      .select('id')
      .eq('user_id', user.id)
      .eq('component_id', agent.id)
      .single()

    isInToolkit = !!toolkit
  }

  const component = agent as Component

  // Fetch reviews
  const [reviews, userReview] = await Promise.all([
    getComponentReviews(agent.id),
    getUserReview(agent.id)
  ])
        ...
        <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            ...
            <AddToToolkitButton
              ...
              redirectPath={`/claude-code/agents/${slug}`}
            />
          </div>
        </div>
      ...
      <div className="space-y-6 mb-8">
        ...
        {component.files && component.files.filter(f => f.path).length > 0 ? (
          ...
          <div className="glass border-border/50 rounded-xl p-8 text-center text-muted-foreground">
            ...
            <p>No content files available for this agent.</p>
          </div>
        )}
        ...
        <div className="glass border-border/50 rounded-xl p-4">
          ...
          <div className="bg-muted/50 rounded-lg p-3 font-mono text-sm">
            <code>claude --agent {component.slug}</code>
          </div>
          <p className="text-sm text-muted-foreground mt-3 mb-2">
            Install path:
          </p>
          <div className="bg-muted/50 rounded-lg p-3 font-mono text-sm">
            <code>~/.claude/agents/{component.slug}/</code>
          </div>
        </div>
      </div>
