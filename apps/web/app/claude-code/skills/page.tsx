...
export default async function SkillsPage({ searchParams }: SkillsPageProps) {
  ...
  const { data: categories } = await supabase
    ...
    .select('*')
    .or('component_type.eq.skill,component_type.is.null')
    .order('sort_order')
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <Badge variant="secondary">{skills?.length || 0}</Badge>
        </div>

        <p className="text-muted-foreground">
          Specialized knowledge and workflows that extend Claude&apos;s capabilities
        </p>
      </div>
      ...
      <div className="glass border-border/50 rounded-xl p-4 mb-8">
        <form className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {categories?.map((cat) => (
              <Link key={cat.slug} href={`/claude-code/skills?category=${cat.slug}`}>
                <Badge
      ...
      {skills && skills.length > 0 ? (
        <ComponentGrid
          components={skills as Component[]}
